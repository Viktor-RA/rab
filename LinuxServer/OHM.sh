...
# $2 - Поиск элементов вывод JSON data, info (discoveryTemp - поиск датчиков температуры, discoveryFun - поиск вентиляторов, discoveryPower - поиск датчиков напряжений)
...
then
  ...
  obrtabl=$(echo "${get}" 2>/dev/null|awk NR\>1|grep -E 'Temperature'|sed -E 's/  ([^ ])/\t\1/g; s/^1/1\t/;'|awk -F\\t '{print $3"  "$6"  "$7"  "$8"  "$9}')
...
then
  ...
  # А если нужно выбрать только строки с определеными параметрами например температуру напряжение
  obrtabl=$(echo "${get}" 2>/dev/null|awk NR\>1|grep -E 'Control'|sed -E 's/  ([^ ])/\t\1/g; s/^1/1\t/;'|awk -F\\t '{print $3"  "$6"  "$7"  "$8"  "$9}')
...
then
  ...
  # А если нужно выбрать только строки с определеными параметрами например температуру напряжение
  obrtabl=$(echo "${get}" 2>/dev/null|awk NR\>1|grep -E 'Voltage'|sed -E 's/  ([^ ])/\t\1/g; s/^1/1\t/;'|awk -F\\t '{print $3"  "$6"  "$7"  "$8"  "$9}')
